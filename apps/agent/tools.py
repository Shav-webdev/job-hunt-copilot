...
AI_CORE_URL = os.environ.get("AI_CORE_URL", "http://jobhunt-ai-core:8000")
...
def make_tools(api_token: str):
            ...
            for feed_url in JOB_RSS_FEEDS:
                try:
                    ...
                    for entry in feed.entries:
                        ...
                        if query.lower() not in title.lower() and query.lower() not in entry.get("summary", "").lower():
                            continue
                        jobs.append({
                            "title": title,
                            "company": entry.get("author", entry.get("tags", [{}])[0].get("term", "Unknown") if entry.get("tags") else "Unknown"),
                            "url": entry.get("link", ""),
    ...
    async def score_job(cv_text: str, job_description: str) -> str:
        ...
        if not r.is_success:
            return json.dumps({"error": "Scoring failed."})
        return r.text

    @tool
    async def save_application(job_title: str, company: str, job_url: str, notes: str = "") -> str:
        """Save a job and create an application entry in the database."""
        async with httpx.AsyncClient(timeout=15) as client:
            job_r = await client.post(f"{API_URL}/jobs", headers=auth_headers, json={
                "title": job_title,
                "company": company,
            ...
            app_r = await client.post(f"{API_URL}/applications", headers=auth_headers, json={
                "jobId": job_id,
                "status": "saved",
                "notes": notes,
            })
    ...
    @tool
    async def draft_cover_letter(cv_text: str, job_title: str, job_description: str, company: str) -> str:
        """Draft a cover letter for a specific job using the candidate's CV.
        Returns the complete cover letter text."""
        async with httpx.AsyncClient(timeout=60) as client:
            async with client.stream("POST", f"{AI_CORE_URL}/draft-cover-letter", json={
                "cv_text": cv_text[:5000],
                "job_title": job_title,
                "job_description": job_description[:3000],
                "company": company,
                ...
                async for line in r.aiter_lines():
                    ...
                    try:
                        ...
                        continue
        return text or "Cover letter generation failed."
